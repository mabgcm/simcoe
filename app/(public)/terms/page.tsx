...
export function generateMetadata(): Metadata {
  ...
  return { title: messages.meta.terms };
}
...
export default function TermsPage() {
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
      <h1 className="font-heading text-5xl text-secondary">{t.termsTitle}</h1>
      ...
      <div className="prose-content mt-8 rounded-lg border bg-white p-6 shadow-sm">
        {t.termsBody.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
      </div>
    </section>
  );
}
